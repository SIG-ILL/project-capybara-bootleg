...
pcb::LayeredHeightmap::LayeredHeightmap(int width, int height, const std::vector<unsigned char>& elevationValues) : Heightmap(width, height, elevationValues), layers(1, std::make_shared<HeightmapLayer>(std::make_shared<Heightmap>(width, height, elevationValues), pcb::LayerMode::Addition)) {}

pcb::LayeredHeightmap::LayeredHeightmap(const pcb::Heightmap& baseHeightmap) : Heightmap(baseHeightmap), layers(1, std::make_shared<HeightmapLayer>(std::make_shared<Heightmap>(baseHeightmap), pcb::LayerMode::Addition)) {}
...
std::vector<std::shared_ptr<pcb::HeightmapLayer>> pcb::LayeredHeightmap::getLayers() const {
	return layers;
}